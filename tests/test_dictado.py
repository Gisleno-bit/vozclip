...
# ===========================================================================
# El fallo de los vídeos: Vosk cierra la frase en una pausa y se perdía
# ===========================================================================


class _ReconocedorComoVosk:
    """Imita al KaldiRecognizer real: devuelve parciales mientras se habla,
    cierra la frase (AcceptWaveform True) al oír una pausa, y después de eso
    FinalResult viene vacío porque ya se vació al cerrar."""

    def __init__(self, *frases_con_pausas):
        self.frases = list(frases_con_pausas)
        self.palabras = self.frases.pop(0).split() if self.frases else []
        self.i = 0
        self.parcial = ""
        self.cerrado = ""
    ...
    def AcceptWaveform(self, _trozo):
        if self.i < len(self.palabras):
            self.i += 1
            self.parcial = " ".join(self.palabras[: self.i])
            return False
        if self.parcial:                                   # pausa: cierre
            self.cerrado, self.parcial = self.parcial, ""
            if self.frases:                                # siguiente frase
                self.palabras = self.frases.pop(0).split()
                self.i = 0
            return True
        return False

    def Result(self):
        r = _json.dumps({"text": self.cerrado})
        self.cerrado = ""
        return r

    def PartialResult(self):
        return _json.dumps({"partial": self.parcial})

    def FinalResult(self):
        return _json.dumps({"text": self.parcial})       # solo lo que quede en curso


def _motor_vosk_simulado(*frases):
    motor = dictado.MotorVosk.__new__(dictado.MotorVosk)
    motor._reconocedor = _ReconocedorComoVosk(*frases)
    motor._frases = []
    motor._ultimo_mostrado = ""
    return motor


def test_una_frase_cerrada_por_pausa_no_se_pierde():
    """Vídeo 1: 'domingo por sábado', pausa, y el final llegaba vacío."""
    motor = _motor_vosk_simulado("domingo por sábado")
    for _ in range(6):
        motor.alimentar(b"x")
    assert motor.finalizar() == "domingo por sábado"


def test_los_parciales_se_ven_mientras_se_habla():
    motor = _motor_vosk_simulado("domingo por sábado")
    vistos = [p for p in (motor.alimentar(b"x") for _ in range(3)) if p]
    assert vistos == ["domingo", "domingo por", "domingo por sábado"]


def test_varias_frases_con_pausas_se_acumulan():
    """'F1 da muy poco tiempo': cada pausa perdía lo anterior. Ahora se
    puede dictar con calma, párrafo a párrafo."""
    motor = _motor_vosk_simulado("el domingo fuimos", "a comer paella", "y luego al cine")
    for _ in range(20):
        motor.alimentar(b"x")
    assert motor.finalizar() == "el domingo fuimos a comer paella y luego al cine"


def test_lo_mostrado_incluye_lo_cerrado_y_lo_en_curso():
    motor = _motor_vosk_simulado("el domingo", "fuimos")
    mostrados = [p for p in (motor.alimentar(b"x") for _ in range(6)) if p]
    assert "el domingo fuimos" in mostrados[-1]


def test_finalizar_reinicia_el_acumulado():
    motor = _motor_vosk_simulado("uno")
    for _ in range(3):
        motor.alimentar(b"x")
    assert motor.finalizar() == "uno"
    assert motor.finalizar() == ""


def test_el_servicio_usa_el_parcial_si_el_final_llega_vacio():
    """Cinturón y tirantes: 'prohibido no he entendido si hubo texto'."""

    class MotorQuePierdeElFinal:
        def iniciar(self):
            pass

        def alimentar(self, _t):
            return "texto que se vio"
    ...
    servicio = dictado.ServicioDictado(
        notificar=lambda t, d: eventos.append((t, d)),
        ajustes={"activado": True},
        fabrica_motor=MotorQuePierdeElFinal,
        fabrica_captura=lambda: dictado.CapturaFalsa([b"\x10\x00" * 4000] * 3),
    )
    servicio.empezar()
    import time as _t
    ...
    while servicio.activo and _t.monotonic() < plazo:
        _t.sleep(0.02)
    assert ("texto", "texto que se vio") in eventos
    assert not any(t == "error" for t, _ in eventos)
