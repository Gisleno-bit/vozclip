...
# ===========================================================================
def _hay_tkinter_usable() -> bool:
    """¿Se puede abrir una ventana AQUÍ Y AHORA?

    No basta con mirar el sistema operativo. En los runners de Windows, el
    Python del tool cache trae a veces un Tcl/Tk incompleto y `tk.Tk()`
    falla con "Can't find a usable init.tcl". El autotest abre una ventana,
    así que sin esta comprobación esos tests fallaban en Windows por un
    problema del entorno, no del proyecto.
    """
    import subprocess
    import sys

    codigo = "import tkinter; r=tkinter.Tk(); r.withdraw(); r.destroy()"
    try:
        return subprocess.run(
            [sys.executable, "-c", codigo], capture_output=True, timeout=60
        ).returncode == 0
    except Exception:
        return False
...
def _marcas_del_autotest() -> dict[str, str]:
    ...
    if not _hay_tkinter_usable():
        pytest.skip(
            "Tkinter no puede abrir ventana aquí (falta display o Tcl/Tk "
            "roto): el autotest necesita una."
        )
...
def test_las_marcas_sobreviven_a_una_codificacion_equivocada():
    ...
    if not _hay_tkinter_usable():
        pytest.skip("Tkinter no puede abrir ventana aquí: el autotest la necesita")
