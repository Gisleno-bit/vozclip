...
# ===========================================================================
# Corrección en mitad del dictado
# ===========================================================================
PARRAFO_JULIAN = "Come alpiste mientras está bailando con un cisne"
AMB_J = (0, len(PARRAFO_JULIAN))


def test_con_verbo_y_palabra_presente_es_correccion():
    o = c.orden_durante_dictado("cambia alpiste por rueda", PARRAFO_JULIAN, AMB_J)
    assert o is not None and o.tipo == "cambiar" and o.con_verbo


def test_sin_verbo_es_texto():
    """'fue por pan' también parecería una orden si 'fue' está en el texto."""
    assert c.orden_durante_dictado("alpiste por rueda", PARRAFO_JULIAN, AMB_J) is None


def test_con_verbo_pero_palabra_ausente_es_texto():
    assert c.orden_durante_dictado("cambia rumbo por completo", PARRAFO_JULIAN, AMB_J) is None


def test_prosa_que_empieza_por_cambia_es_texto():
    assert c.orden_durante_dictado("cambia de opinión y se va", PARRAFO_JULIAN, AMB_J) is None


def test_borrar_con_verbo_es_correccion():
    o = c.orden_durante_dictado("borra cisne", PARRAFO_JULIAN, AMB_J)
    assert o is not None and o.tipo == "borrar"


def test_por_numero_con_verbo_es_correccion():
    o = c.orden_durante_dictado("sustituye la dos por rueda", PARRAFO_JULIAN, AMB_J)
    assert o is not None and o.indices == [2]


def test_numero_fuera_de_rango_es_texto():
    assert c.orden_durante_dictado("cambia la cincuenta por x", PARRAFO_JULIAN, AMB_J) is None


def test_interpretar_orden_marca_el_verbo():
    assert c.interpretar_orden("cambia casa por cosa").con_verbo is True
    assert c.interpretar_orden("casa por cosa").con_verbo is False
    assert c.interpretar_orden("borra casa").con_verbo is True


# ===========================================================================
# Un número dicho como palabra (la fecha de Julián)
# ===========================================================================
FECHA = "Seis del nueve del dos mil veintiséis comeremos espagueti"
AMB_F = (0, len(FECHA))


def test_veintiseis_es_la_palabra_si_esta_en_el_texto():
    """Julián dictó una fecha y quiso cambiar 'veintiséis'. Antes se leía
    como 'la palabra número 26' y decía que no había tantas."""
    r = c.aplicar(FECHA, c.interpretar_orden("corrige veintiséis por treinta y seis"), AMB_F)
    assert r.aplicado
    assert "dos mil treinta y seis comeremos" in r.texto


def test_con_articulo_sigue_siendo_posicion():
    r = c.aplicar(FECHA, c.interpretar_orden("cambia la dos por tarde"), AMB_F)
    assert r.aplicado
    assert r.texto.startswith("Seis tarde nueve")


def test_sin_articulo_y_sin_la_palabra_es_posicion():
    """'cambia cuatro por X' en un texto sin 'cuatro': la cuarta palabra."""
    r = c.aplicar(FECHA, c.interpretar_orden("cambia cuatro por X"), AMB_F)
    assert r.aplicado
    assert r.texto == "Seis del nueve X dos mil veintiséis comeremos espagueti"


def test_numero_literal_tambien_durante_el_dictado():
    o = c.orden_durante_dictado("corrige veintiséis por treinta y seis", FECHA, AMB_F)
