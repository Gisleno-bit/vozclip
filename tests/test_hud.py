...
# ===========================================================================
# Los vídeos de Julián, de principio a fin
# ===========================================================================
class _ReconocedorComoVoskHud:
    def __init__(self, frase):
    ...
    def AcceptWaveform(self, _):
        if self.i < len(self.palabras):
            self.i += 1
            self.parcial = " ".join(self.palabras[: self.i])
            return False
        if self.parcial:
            self.cerrado, self.parcial = self.parcial, ""
            return True
        return False
    ...
    def PartialResult(self):
        ...
        return _j.dumps({"partial": self.parcial})

    def FinalResult(self):
        import json as _j

        return _j.dumps({"text": ""})


def _motor_como_vosk(frase):
    """Un MotorVosk de verdad, con el reconocedor sustituido: prueba el
    `alimentar` y el `finalizar` reales contra el comportamiento de Vosk."""
    from vozclip import dictado as moddictado

    class MotorVoskSimulado(moddictado.MotorVosk):
        def __init__(self):
        ...
        def iniciar(self):
            self._reconocedor = _ReconocedorComoVoskHud(frase)
            self._frases = []
            self._ultimo_mostrado = ""

    return MotorVoskSimulado()


@pytest.fixture
def hud_video():
    """HUD con un motor que se comporta como Vosk y un micrófono que no se
    calla solo: lo que Julián tiene delante."""
    from vozclip import dictado as moddictado
    from vozclip.hud import HUD
    from vozclip.voz import ServicioVoz
    ...
    def fabrica(notificar):
        return moddictado.ServicioDictado(
            notificar=notificar, ajustes={"activado": True, "modelo": "fingido"},
            fabrica_motor=lambda: _motor_como_vosk(frases.pop(0)),
            fabrica_captura=lambda: _CapturaComoElMicrofono(), voz=voz,
        )
...
def test_video_1_f9_domingo_por_sabado(hud_video):
    """Vídeo 1: F9, 'domingo por sábado', pausa. Decía 'no he entendido
    nada' con el texto en pantalla."""
    hud = hud_video
    hud.editor.insert("1.0", "El domingo fuimos a comer paella")
    hud.editor.mark_set("insert", "end-1c")
    hud.ajustes["correccion"]["parar_tras_silencio"] = 0.6
    hud.frases.append("domingo por sábado")
    ...
    assert hud._texto() == "El sábado fuimos a comer paella"
    assert hud.correccion is None
    assert not any("No he entendido nada" in t for t in hud.voz._motor.dicho)


def test_video_2_f1_tras_un_f9_no_queda_atascado_en_correccion(hud_video):
    """Vídeo 2: con el editor vacío, un F1 mostraba CORRIGIENDO: el estado
    de corrección se había quedado de un F9 fallido."""
    hud = hud_video
    hud.correccion = {"ambito": (0, 0), "fase": "orden", "opciones": None,
                      "orden": None, "elegidas": None}          # atascado
    hud.frases.append("domingo en la tarde y fuimos a los coches de choque")

    hud.accion_dictar()
    _bombear(hud, 0.4)
    assert hud.correccion is None                         # F1 lo limpia
    assert hud.etiqueta_dictado.cget("text").startswith("● ESCUCHANDO")
    _bombear(hud, 2.5)
    hud.accion_dictar()
    _bombear(hud, 1.5)
    assert hud._texto() == "Domingo en la tarde y fuimos a los coches de choque"


def test_un_error_de_microfono_cierra_la_correccion(hud_video):
    hud = hud_video
    hud.editor.insert("1.0", "Texto.")
    hud.correccion = {"ambito": (0, 6), "fase": "orden", "opciones": None,
                      "orden": None, "elegidas": None}
    hud.voz._motor.dicho.clear()
    hud._procesar_evento_dictado("error", "No he entendido nada. Prueba otra vez.")
    hud.voz.esperar_silencio(limite=2)
    assert hud.correccion is None
    assert any("efe nueve" in t for t in hud.voz._motor.dicho)
    assert not any("No he entendido nada" in t for t in hud.voz._motor.dicho)


def test_dictar_con_pausas_largas_no_pierde_nada(hud_video):
    """'F1 da muy poco tiempo': tres frases con pausa entre ellas."""
    hud = hud_video
    hud.frases.append("el domingo fuimos a comer paella")
    hud.accion_dictar()
    _bombear(hud, 3.0)              # sobra tiempo: Vosk cierra y espera
    hud.accion_dictar()
    _bombear(hud, 1.5)
    assert hud._texto() == "El domingo fuimos a comer paella"


def test_orden_no_entendida_dice_lo_oido(hud_corrector):
    """'Prohibido no he entendido nada si hubo texto': se repite lo oído
    y se da la pista."""
    hud = hud_corrector
    hud.editor.insert("1.0", "Aquella noche.")
    hud.editor.mark_set("insert", "end-1c")
    hud.voz._motor.dicho.clear()
    hud.respuestas.append("qué tal estás")
    hud.respuestas.append("cancela")
    hud.accion_corregir()
    _escuchar(hud)
    _escuchar(hud)
    dicho = " ".join(hud.voz._motor.dicho)
    assert "He oído: qué tal estás" in dicho
    assert "cambia, la palabra, por, la buena" in dicho
    assert "No he entendido nada" not in dicho
