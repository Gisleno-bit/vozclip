...
def _interpretar_objetivo(texto: str) -> tuple[str, list[int]]:
    ...
    con_articulo = recortado != sin          # había "la", "palabra", "número"...
    ...
    if numeros and solo_numeros:
        if con_articulo:
            return "", numeros           # "la tres": inequívocamente un número
        # "veintiséis" a secas puede ser la palabra 26 del párrafo O la
        # palabra "veintiséis" escrita en el texto (una fecha, una edad).
        # Se devuelven las dos lecturas; `aplicar` prueba primero la
        # literal, porque si esa palabra está en el texto, es lo que
        # el usuario ve y oye.
        return limpio, numeros
...
def orden_durante_dictado(dicho: str, texto: str, ambito: tuple[int, int]) -> Orden | None:
    ...
    if orden.tipo not in ("cambiar", "borrar") or not orden.con_verbo:
        return None
    if orden.buscar and localizar(texto, orden.buscar, ambito):
        return orden
    if orden.indices and por_indices(texto, orden.indices, ambito):
        return orden
    return None
...
def aplicar(texto: str, orden: Orden, ambito: tuple[int, int], eleccion: int | None = None) -> Resultado:
    ...
    # Un número dicho sin artículo ("veintiséis") lleva las dos lecturas:
    # la palabra literal y la posición. Si la palabra está en el texto,
    # gana la literal.
    if orden.buscar and orden.indices:
        if localizar(texto, orden.buscar, ambito):
            orden.indices = []
        else:
            orden.buscar = ""
