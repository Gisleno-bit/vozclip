...
class MotorVosk:
    ...
    def iniciar(self) -> None:
        ...
        # Las frases que Vosk ya ha cerrado en esta escucha. Es la memoria
        # que faltaba: sin ella, cada pausa borraba lo dicho hasta entonces.
        self._frases: list[str] = []
        self._ultimo_mostrado = ""

    def alimentar(self, trozo: bytes) -> str | None:
        """Devuelve lo reconocido hasta ahora, para mostrarlo; None si no
        ha cambiado desde la última vez.

        =====================================================================
        EL FALLO QUE HABÍA AQUÍ
        =====================================================================
        Vosk cierra una frase cuando oye una pausa: `AcceptWaveform`
        devuelve True y el texto está en `Result()`. Antes, ese texto se
        devolvía solo para enseñarlo en pantalla y NO SE GUARDABA. Después,
        `finalizar()` llamaba a `FinalResult()`, que ya venía vacío. Todo
        lo que Vosk cerraba por una pausa se veía y se tiraba. Solo
        sobrevivía lo dicho sin respirar antes de pulsar F1.

        Ahora cada frase cerrada se acumula, y lo que se muestra es lo
        acumulado más el parcial en curso: se ve escribir mientras se
        habla, y las pausas no pierden nada.
        =====================================================================
        """
        ...
        if self._reconocedor.AcceptWaveform(trozo):
            texto = json.loads(self._reconocedor.Result()).get("text", "").strip()
            if texto:
                self._frases.append(texto)
            en_curso = ""
        else:
            en_curso = json.loads(self._reconocedor.PartialResult()).get("partial", "").strip()

        mostrado = " ".join(self._frases + ([en_curso] if en_curso else [])).strip()
        if mostrado == self._ultimo_mostrado:
            return None
        self._ultimo_mostrado = mostrado
        return mostrado or None

    def finalizar(self) -> str:
        """Todo lo reconocido en esta escucha: las frases cerradas más lo que
        quedara en curso."""
        if self._reconocedor is None:
            return ""
        resto = json.loads(self._reconocedor.FinalResult()).get("text", "").strip()
        if resto:
            self._frases.append(resto)
        texto = " ".join(self._frases).strip()
        self._frases = []
        self._ultimo_mostrado = ""
        return texto
...
class ServicioDictado:
    ...
    def _grabar(self) -> None:
        ...
        try:
            ...
            if not texto.strip() and ultimo_parcial.strip():
                # El motor ha perdido el final pero se vio texto reconocido:
                # se usa lo visto. Nunca "no he entendido" con texto en pantalla.
                texto = ultimo_parcial
            self._pitido(660, 90)
