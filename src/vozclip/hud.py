...
class HUD:
        ...
        if tipo == "inicio":
            ...
            if self.correccion is not None:
                self.etiqueta_dictado.config(
                    text="● CORRIGIENDO — di el cambio y calla", fg=self.tema["corregir"]
                )
            else:
                self.etiqueta_dictado.config(
            ...
            if self.correccion is not None:
                # Se cierra la corrección: dejarla abierta enrutaba todo el
                # dictado posterior a ella (los F1 mostraban "corrigiendo").
                self.correccion = None
                self.etiqueta_dictado.config(
                    text="Corrección: no he oído la orden", fg=self.tema["escucha"],
                )
                self.voz.hablar(
                    "No he oído ninguna orden. Pulsa efe nueve y di, por ejemplo: "
    ...
    def accion_dictar(self) -> None:
        ...
        # F1 es dictado, sin excepción. Si quedaba una corrección a medias
        # (por ejemplo, F9 falló y nadie la cerró), se descarta: si no, lo
        # dictado se enrutaba a la corrección para siempre.
        if self.correccion is not None:
            self.correccion = None
        ...
        self.servicio_dictado.empezar(
            parar_tras_silencio=float(self.ajustes.get("dictado", {}).get("parar_tras_silencio", 0))
        )
    ...
    def _aplicar_correccion(self, dicho: str) -> None:
        ...
        if orden.tipo == "desconocida":
            # Nunca "no he entendido nada" con texto reconocido: se dice lo
            # oído y cómo convertirlo en una orden.
            estado["fase"] = "orden"
            self.voz.hablar(
                f"He oído: {dicho}. Para cambiar una palabra di: cambia, "
                "la palabra, por, la buena. O di léelo."
            )
            self._escuchar_orden()
            return
        self._resolver_orden(orden)
