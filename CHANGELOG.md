...
## 2.14.0 — Lo que Vosk cerraba en una pausa se tiraba

### Lo que enseñaron los dos vídeos

Vídeo 1: F9, «domingo por sábado», el parcial en pantalla, y a los dos
segundos «no he entendido nada». Vídeo 2: con el editor VACÍO, un F1
normal mostraba «CORRIGIENDO».

### Causa raíz 1: el motor perdía cada frase cerrada por una pausa

Vosk cierra una frase cuando oye una pausa: `AcceptWaveform` devuelve True
y el texto está en `Result()`. `MotorVosk.alimentar` devolvía ese texto
**solo para enseñarlo** y no lo guardaba en ningún sitio. Después,
`finalizar()` llamaba a `FinalResult()`, que ya venía vacío. **Todo lo que
Vosk cerraba por una pausa se veía en pantalla y se tiraba.** Solo
sobrevivía lo dicho sin respirar antes de pulsar F1.

Esto explica los dos síntomas de Julián: la orden de F9 que «detecta pero
no aplica» (la parada automática llegaba tras el cierre de Vosk, y el final
estaba vacío), y «F1 da muy poco tiempo» (cualquier pausa borraba lo
anterior). Y de rebote, los «no he entendido nada» que llevaban semanas.

Ahora cada frase cerrada se acumula; lo mostrado es lo acumulado más el
parcial en curso (se ve escribir mientras se habla); y `finalizar` devuelve
todo. Además, si un motor devolviera el final vacío con texto visto, el
servicio usa el parcial: nunca «no he entendido nada» con texto en pantalla.

### Causa raíz 2: la corrección se quedaba atascada

Si F9 acababa en error, `self.correccion` no se limpiaba. Desde ese
momento **todo dictado** se enrutaba a la corrección: por eso un F1 con el
editor vacío decía «CORRIGIENDO». Ahora F1 es dictado sin excepción y
descarta cualquier corrección pendiente; un error del micrófono cierra la
corrección con un mensaje que dice qué hacer.

### Además

- Orden no entendida en F9: «He oído: tal. Para cambiar una palabra di:
  cambia, la palabra, por, la buena». Nunca «no he entendido nada» con
  texto reconocido.
- La etiqueta dice CORRIGIENDO desde el principio del F9, no solo con el
  parcial.
- `dictado.parar_tras_silencio` (0 por defecto): para quien prefiera que
  F1 termine solo tras unos segundos de silencio.
- Tests con un reconocedor que imita el comportamiento exacto de Vosk
  (parciales, cierre por pausa, `FinalResult` vacío): los dos vídeos de
  principio a fin, varias frases con pausas, el estado atascado, el error
  de micrófono, la orden no entendida.
